...
class NavBar extends React.Component {
    ...
    render() { 
        const { toggleNavbar, showNavbar, userLogged, logOut} = this.props
        ...
        <button onClick={toggleNavbar} className="closebtn">&times;</button>
            ...
            {userLogged ? <h1 onClick={() => {logOut(); toggleNavbar()}; localStorage.clear()}><Link to={"/"}>| Log Out |</Link></h1> : <h1 onClick={toggleNavbar}><Link to={"/login"}>| Log In |</Link></h1>}
